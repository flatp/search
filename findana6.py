...
pop_news = pd.read_csv('pop_news_all_true.csv')
...
grouped_keys = set(grouped2_dropped['name_x'])
...
cols_to_fill = [col for col in grouped2_dropped.columns if col != 'name_x']
...
grouped2_expanded = pd.concat([grouped2_dropped, df_fill], ignore_index=True)

# ==== 人気度・知名度変化とのマージ ====
final_df = pd.merge(grouped2_expanded, pop_all, left_on='name_x', right_on='name', how='left')
final_df = final_df[['name', 'burst_count', 'burst_views_sum', 'news_count', 'pop_diff', 'aware_diff', '202302人気度', '202302知名度']]

# ==== 結果をCSV出力 ====
final_df.to_csv('burst_news_sentiment_pop_change2.csv', index=False)
...
plt.subplot(2, 2, 1)
sns.regplot(data=final_df, x='burst_views_sum', y='pop_diff')
plt.title('バースト合計 vs 人気度の変化')

plt.subplot(2, 2, 2)
sns.regplot(data=final_df, x='burst_views_sum', y='aware_diff')
plt.title('バースト合計 vs 知名度の変化')

plt.subplot(2, 2, 3)
sns.regplot(data=final_df, x='burst_views_sum', y='202302人気度')
plt.title('バースト合計 vs 人気度')

plt.subplot(2, 2, 4)
sns.regplot(data=final_df, x='burst_views_sum', y='202302知名度')
plt.title('バースト合計 vs 知名度')
...
plt.subplot(2, 2, 1)
sns.regplot(data=final_df, x='burst_count', y=np.abs(final_df['pop_diff']))
plt.title('バースト件数 vs 人気度の変化(絶対値)')

plt.subplot(2, 2, 2)
sns.regplot(data=final_df, x='burst_count', y=np.abs(final_df['aware_diff']))
plt.title('バースト件数 vs 知名度の変化(絶対値)')

plt.subplot(2, 2, 3)
sns.regplot(data=final_df, x='news_count', y=np.abs(final_df['pop_diff']))
plt.title('ニュース件数 vs 人気度の変化(絶対値)')

plt.subplot(2, 2, 4)
sns.regplot(data=final_df, x='news_count', y=np.abs(final_df['aware_diff']))
plt.title('ニュース件数 vs 知名度の変化(絶対値)')
...
plt.subplot(2, 2, 1)
sns.regplot(data=final_df, x='burst_count', y='202302人気度')
plt.title('バースト件数 vs 人気度')

plt.subplot(2, 2, 2)
sns.regplot(data=final_df, x='burst_count', y='202302知名度')
plt.title('バースト件数 vs 知名度')

plt.subplot(2, 2, 3)
sns.regplot(data=final_df, x='news_count', y='202302人気度')
plt.title('ニュース件数 vs 人気度')

plt.subplot(2, 2, 4)
sns.regplot(data=final_df, x='news_count', y='202302知名度')
plt.title('ニュース件数 vs 知名度')
...
print(final_df[['burst_count', 'news_count', 'pop_diff', 'aware_diff', '202302人気度', '202302知名度']].corr())
